...
class QuizControllerTest {
    ...
    public void getQuiz1() throws Exception {
        ...
        final ResultActions result = mockMvc.perform(get(url)
                .param("code","1")              //  value 가 "1"인 이유는 JSON 이라서
        );
        //  then
    ...
    public void getQuiz2() throws Exception {
        ...
        final String url = "/quiz";
        // when
        final ResultActions result = mockMvc.perform(get(url)   //  getQuiz1() 에서 이미 get() import 되어 있기 때문
                .param("code","2")
        );
        //  then
...
/*
    ...
    Code code = new Code(13)        //  QuizController.java
    objectMapper.writeAsString(code)

    코드 분석
    new code(13) 을 Code 클래스의 code 객체를 생성했고, WriteValueAsString(code) 메서드를 통해서 JSON 문자열 형태로 객체가 변환

    이런식으로 객체 → JSON 문자열로의 변환을 '객체 직렬화' 라고 하기도 함.
